                    ...
                    <div className="flex flex-col items-center gap-2 w-full">
                        {navItems.map(item => (
                            <div key={item.id} className="group relative w-full flex justify-center">
                                <Tooltip>
                                    <TooltipTrigger asChild>
                                        <Button variant={'ghost'} size="icon" className={cn( 'h-12 w-12 rounded-full bg-card transition-all duration-200 hover:rounded-2xl hover:bg-primary', activeView === item.id && 'rounded-2xl bg-primary' )} onClick={() => handleViewChange(item.id as StaffView)}>
                                            {item.icon}
                                        </Button>
                                    </TooltipTrigger>
